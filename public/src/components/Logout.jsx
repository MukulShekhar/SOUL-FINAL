...
function Logout() {
  ...
  const handleClick = async () => {
    try {
      const userStr = localStorage.getItem(LOCALHOST_KEY);
      const id = userStr ? JSON.parse(userStr)._id : null;
      if (id) {
      ...
      // Fallback: clear and navigate if no id or non-200
      localStorage.removeItem(LOCALHOST_KEY);
      setLoggedOut(true);
      navigate("/login");
    } catch (err) {
      // Ensure user is logged out even if request fails
      localStorage.removeItem(LOCALHOST_KEY);
      setLoggedOut(true);
      navigate("/login");
    }
